...
namespace SubRecognizer
{
    ...
    public class RubineRecognizer : ISerializable
    {
        ...
        #region Constructors
        public RubineRecognizer(Matrix weights, List<double> weights_0, List<string> classNames)
        {
            _weights = weights;
        ...
        public RubineRecognizer(SerializationInfo info, StreamingContext ctxt)
        {
            ...
            _weights = (Matrix)info.GetValue("weights", typeof(Matrix));
    ...
    public class RubineRecognizerUpdateable : ISerializable
    {
        ...
        {
            ccm = new Matrix(1, 1);
            ccmInverse = new Matrix(1, 1);
            weights = new Matrix(1, 1);
        ...
        {
            this.ccm = new Matrix(this.classes[0].CovarianceMatrix.RowCount, this.classes[0].CovarianceMatrix.ColumnCount);
            this.ccmInverse = new Matrix(this.classes[0].CovarianceMatrix.RowCount, this.classes[0].CovarianceMatrix.ColumnCount);
            this.weights = new Matrix(this.classes.Count, this.classes[0].CovarianceMatrix.RowCount);
            this.weights_0 = new List<double>(this.classes.Count);
        }

        private void updateWeightsSize()
        {
            this.weights = new Matrix(this.classes.Count, this.classes[0].CovarianceMatrix.RowCount, 0.0);
        ...
        private void calculateCCM()
        {
            ...
            for (int i = 0; i < classes[0].Averages.Count; i++)
            {
                for (int j = 0; j < classes[0].Averages.Count; j++)
                {
                    sumC = 0.0;
                    for (int c = 0; c < classes.Count; c++)
                        sumC += classes[c].CovarianceMatrix[i, j];

                    if (denom > 0)
                        this.ccm[i, j] = sumC / (double)denom;
                    else
                        this.ccm[i, j] = 0.0;
                }
            }
        ...
        private void calculateWeights()
        {
            ...
            for (int c = 0; c < this.classes.Count; c++)
            {
                for (int j = 0; j < this.classes[c].Averages.Count; j++)
                {
                    ...
                    {
                        sumI += this.ccmInverse[i, j] * this.classes[c].Averages[i];
                    }
                    this.weights[c, j] = sumI;
                }
            }
        }
        ...
        private void calculateWeights_0()
        {
            ...
            for (int c = 0; c < classes.Count; c++)
            {
                ...
                for (int i = 0; i < this.classes[c].Averages.Count; i++)
                {
                    sumI += this.weights[c, i] * this.classes[c].Averages[i];
                }
        ...
        {
            //Get the values from info and assign them to the appropriate properties
            ccm = (Matrix)info.GetValue("ccm", typeof(Matrix));
            ccmInverse = (Matrix)info.GetValue("ccmInverse", typeof(Matrix));
    ...
    public class RubineClass
    {
        ...
        public RubineClass()
        {
            ...
            covarianceMatrix = new Matrix(1, 1);
        }
        ...
        public RubineClass(string name, List<Sketch.Point> points)
        {
            ...
            calculateAverages();
            this.covarianceMatrix = new Matrix(this.averages.Count, this.averages.Count, 0.0);
        }
        #endregion
        ...
        {
            if (this.covarianceMatrix == null || this.covarianceMatrix.RowCount != this.averages.Count)
                this.covarianceMatrix = new Matrix(this.averages.Count, this.averages.Count, 0.0);
            ...
            for (int i = 0; i < numFeatures; i++)
            {
                for (int j = 0; j < numFeatures; j++)
                {
                    ...
                    this.covarianceMatrix[i, j] = sumE;
                }
            }
        }
        #endregion
